...
async function saveBingoCard(req: any, res: any) {
    let body = wmsrv.wm.protobuf.LoadBingoStats.decode(req.body);
    ...
    let bingoCard: any = await prisma.bingoCard.create({
        data: {
            carId: body.carId,
            ramp: rampVal,
            path: pathVal,
    ...
    let msg: any = {
        ...
        ramp: bingoCard.ramp,
        path: bingoCard.path,
        ghosts: bingoCard.ghosts.map((ghost: any) => wm.wm.protobuf.GhostCar.create({
            car: {
                carId: ghost.carId,
    ...
    let message = wmsrv.wm.protobuf.LoadBingoStats.encode(msg);
    common.sendResponse(message, res);
}

async function loadBingoCard(req: any, res: any) {
    let body = wmsrv.wm.protobuf.LoadBingoStats.decode(req.body);
...
async function existingBingoCard(req: any, res: any) {
    let body = wmsrv.wm.protobuf.LoadBingoStats.decode(req.body);
